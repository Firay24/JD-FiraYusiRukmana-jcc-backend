...
export class ActivityController {
  ...
  async uploadFile(@UploadedFile() file: Express.Multer.File, @Req() request: Request, @Body('schoolId') schoolId: string, @Body('seasonId') seasonId: string, @Body('regionId') regionId: string) {
    ...
    for (const row of data) {
      ...
      if (!student) {
        ...
        const createdUser = await this.prismaService.user.create({
          data: {
            ...
            Role: {
              connect: { Id: roleIdParticipant.Id },
            },
          },
        });
        ...
        const idMember = (studentCount + 1).toString();
      ...
      const alreadyParticipant = await this.prismaService.competitionParticipant.findFirst({
        where: {
    ...
    return this.utilityService.globalResponse({
      statusCode: 200,
      message: 'Upload and registration completed successfully',
    });
  }
  // #endregion
}
